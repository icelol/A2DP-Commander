...
Эта версия сделана для личного использования и тестирования исправлений, связанных с обнаружением Bluetooth-аудио устройств, обновлением списка устройств на лету и более стабильным переключением профилей A2DP/HFP.

Основной акцент форка:

- корректнее находить новые Bluetooth-аудио устройства после подключения;
- показывать в списке выбора не только устройства, определённые Bluetooth API как гарнитуры, но и активные аудио endpoints Windows, например Bluetooth-колонки, которые отображаются как `Динамики`;
- уменьшить гонки между событиями подключения, обновлением audio endpoints и автоматическим переключением режима;
- сделать интерфейс немного удобнее для повседневной проверки и отладки.

## Изменения в этом форке

- Добавлено последовательное выполнение событий Bluetooth watcher, чтобы `Added`, `Updated` и `Removed` не перетирали состояние друг друга.
- Обновление списка устройств теперь делает реальное повторное сканирование, а не только читает старый кэш.
- При подключении нового устройства оно автоматически добавляется в список и может быть выбрано без перезапуска приложения.
- Добавлен fallback через активные Windows audio endpoints: Bluetooth-колонки, которые Windows показывает как `Динамики`, тоже могут попасть в список выбора.
- Ожидание появления audio endpoints вынесено из UI-потока и стало отменяемым.
- Переключение режимов `Музыка` / `Звонки` сериализовано, чтобы авто-переключение по приложениям и авто-переключение при подключении не конфликтовали.
- При временном отключении устройства сохранённый выбор больше не стирается из настроек.
- Поиск A2DP/HFP стал устойчивее: учитываются не только имена устройств, но и признаки в device id.
- Переключение Bluetooth-адаптера стало безопаснее: целевой адаптер сначала включается и проверяется, а старый отключается после этого.
- Обновлён внешний вид главного окна: увеличено рабочее пространство, поправлены карточки режимов, фон, шапка и вкладки.
- Для Debug-сборки включены `.pdb` символы, чтобы проще отлаживать локальные проблемы.
...
- [О форке](#о-форке)
- [Изменения в этом форке](#изменения-в-этом-форке)
- [Проблема](#проблема)
- [Решение](#решение)
...
- **icelol** - поддержка персонального форка [github.com/icelol/A2DP-Commander](https://github.com/icelol/A2DP-Commander)
- **Claude** (Anthropic)
